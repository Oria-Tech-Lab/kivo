...
function ProjectCard({
  ...
  const ms = margenStyle(proyecto.margen_pct)
  const estado = ESTADO_CONFIG[proyecto.estado]
  const margenS = proyecto.precio_venta_total - proyecto.gasto_real_total

  return (
    <div className="rounded-xl border border-zinc-200 bg-white flex flex-col hover:border-zinc-300 hover:shadow-sm transition-all overflow-hidden">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 flex-wrap mb-1.5">
            <Badge variant="outline" className={cn('text-[11px] shrink-0', estado?.className ?? '')}>
              {estado?.label ?? proyecto.estado}
            </Badge>
            <Badge variant="outline" className="text-[11px] shrink-0 bg-zinc-50 text-zinc-500 border-zinc-200">
              {FASE_LABEL[proyecto.fase] ?? proyecto.fase}
            </Badge>
          </div>
          <Link
            href={`/proyectos/${proyecto.id}`}
            className="font-semibold text-zinc-900 hover:text-zinc-700 hover:underline line-clamp-2 leading-snug text-sm"
          >
            {proyecto.nombre}
          </Link>
        </div>
        {(canEdit || canDelete) && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="rounded p-0.5 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700 transition-colors shrink-0 mt-0.5">
      ...
      {/* 2×2 Metrics grid */}
      <div className="grid grid-cols-2 gap-px bg-zinc-100 border-y border-zinc-100">
        <div className="bg-white px-4 py-3">
          <p className="text-[10px] font-medium uppercase tracking-wide text-zinc-400 mb-0.5">Total proyecto</p>
          <p className="text-sm font-bold tabular-nums text-zinc-800">
            {proyecto.precio_venta_total > 0
              ? formatMoney(proyecto.precio_venta_total)
              : <span className="text-zinc-300">—</span>}
          </p>
        </div>
        <div className="bg-white px-4 py-3">
          <p className="text-[10px] font-medium uppercase tracking-wide text-zinc-400 mb-0.5">Total gastos</p>
          <p className="text-sm font-bold tabular-nums text-zinc-800">
            {proyecto.gasto_real_total > 0
              ? formatMoney(proyecto.gasto_real_total)
              : <span className="text-zinc-300">—</span>}
          </p>
        </div>
        <div className="px-4 py-3" style={{ background: ms.bg }}>
          <p className="text-[10px] font-medium uppercase tracking-wide mb-0.5" style={{ color: ms.color, opacity: 0.8 }}>Margen S/</p>
          <p className="text-sm font-bold tabular-nums" style={{ color: ms.color }}>
            {formatMoney(margenS)}
          </p>
        </div>
        <div className="px-4 py-3" style={{ background: ms.bg }}>
          <p className="text-[10px] font-medium uppercase tracking-wide mb-0.5" style={{ color: ms.color, opacity: 0.8 }}>Margen %</p>
          <p className="text-sm font-bold tabular-nums" style={{ color: ms.color }}>
            {proyecto.margen_pct.toFixed(1)}%
          </p>
        </div>
      ...
      <div className="px-4 py-3 flex items-center gap-2 flex-wrap">
        <span className="text-xs text-zinc-500 min-w-0 truncate flex-1">
          {proyecto.cliente?.nombre ?? <span className="text-zinc-300">Sin cliente</span>}
        </span>
        ...
        <Link
          href={`/proyectos/${proyecto.id}`}
          className="inline-flex items-center gap-0.5 text-xs font-medium text-blue-600 hover:text-blue-700 shrink-0"
        >
